...
if __package__ is None or __package__ == "":
    ...
    from VAE.config_vanilla import get_args, make_exp_dirs
    from VAE.data_preprocessing import build_dataloader
    from VAE.model_vanilla import VanillaVAE
else:
    from .config_vanilla import get_args, make_exp_dirs
...
def save_checkpoint(path, model, optimizer, scaler, epoch, global_step, args, stats_path):
    payload = {
...
def main():
    ...
    print("[TrainVAE] Step 1: Build dataloader", flush=True)
    ...
    print("[TrainVAE] Step 2: Build model + optimizer", flush=True)
    model = VanillaVAE(
    ...
    print("[TrainVAE] Step 3: Resume (optional)", flush=True)
    if args.resume:
    ...
    print("[TrainVAE] Step 4: Training", flush=True)
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for x_img, y_target, _sample_id in pbar:
            x_img = x_img.to(device, non_blocking=True)
            if args.merge_mode == "merge":
                y_target = y_target.to(device, non_blocking=True)
            else:
                y_target = {k: v.to(device, non_blocking=True) for k, v in y_target.items()}
            ...
            with autocast_ctx:
                y_hat, mu, logvar = model(x_img)
                recon = model.reconstruction_loss(y_hat, y_target)
            ...
            pbar.set_postfix(loss=f"{avg_loss:.4f}", recon=f"{avg_recon:.4f}", kl=f"{avg_kl:.4f}", lr=f"{lr:.2e}")
