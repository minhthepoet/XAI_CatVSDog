...
def get_args():
    parser = argparse.ArgumentParser(description="PCA visualization for Vanilla VAE latent space.")
    ...
    parser.add_argument("--svd_solver", type=str, default="auto")
...
def main():
    ...
    pca = PCA(n_components=2, whiten=args.whiten, svd_solver=args.svd_solver)
    z_2d = pca.fit_transform(z)

    out_png = args.out_png or str(exp_root / "logs" / "latent_pca_vanilla.png")
    out_npz = args.out_npz or str(exp_root / "logs" / "latent_pca_vanilla.npz")
    ...
    plt.title(f"Vanilla VAE Latent PCA by {args.label_by} (N={z.shape[0]})")
