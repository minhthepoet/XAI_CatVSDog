...
def get_args():
    parser = argparse.ArgumentParser(description="PCA visualization for CVAE latent space.")
...
def main():
    ...
    reducer = PCA(n_components=2, whiten=args.whiten, svd_solver=args.svd_solver)
    z_2d = reducer.fit_transform(z)

    out_png = args.out_png or str(exp_root / "logs" / "latent_pca.png")
    out_npz = args.out_npz or str(exp_root / "logs" / "latent_pca.npz")
    ...
    plt.title(f"CVAE Latent PCA by {args.label_by} (N={z.shape[0]})")
